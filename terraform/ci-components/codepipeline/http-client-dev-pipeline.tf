module "spg-http-client-dev" {
  ...
  pipeline_name     = local.http_client_dev_pipeline_name
  iam_role_arn      = local.iam_role_arn
  log_group         = local.log_group_name
  tags              = local.tags
  cache_bucket      = local.cache_bucket
  codebuild_name    = local.http_client_dev_pipeline_name
  github_repositories = {
    SourceArtifact = ["hmpps-delius-spg-testing-secure-httpclient", var.dev_branch_name]
  }
  stages = [
    {
      name = "Build"
